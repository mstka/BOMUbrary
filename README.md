...
- **蔵書管理**: ISBNから書誌自動取得（NDL→openBD→Google Books→手動）。表紙は openBD/Google Books から解決（NDLサムネイルはホットリンク不可のため不使用）。**表紙が無い本はアプリからカメラ撮影→Google Drive保存→自動でURL反映**。タグ付け、コンディション、状態（空き/貸出中）、検索・フィルタ
